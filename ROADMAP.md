...
*Last substantive revision: 2026-07-07 (Phase 0c Step 6: full end-to-end per-sweep messenger benchmark complete at lmax=300 for m_group_size in {1,3,5} -- 89.62s/188.08s/354.68s per sweep; m_group_size=5 chosen (matches exact-dense accuracy at validation scale); production validation chain (4x array, lmax=300, real Planck data) launched -- see Phase 0c Step 6).*
...
- [~] **Step 6 — production validation chain** at lmax=300, real Planck data ▶ (started 2026-07-04). Benchmark `use_block_correction=True` cost/`m_group_size` per the note above, then verify ESS ≈ N, C_l agreement with the Phase 0 HMC posterior, and no flat-residual-equivalent pathology.
  ...
  - **Caveat found while writing this up:** the isolated benchmark above only timed the Cholesky *rebuild*, not the full `sample_alm_messenger` call — which also pays `n_messenger_iter` (100 by default) forward/adjoint full-sky SHTs per outer sweep (~17ms each from the same probe timing, i.e. ~3.4s/sweep on its own) plus a per-block triangular solve every inner iteration (not just at rebuild time), so the true per-sweep cost is higher than 2.73s at `m_group_size=5`. `scripts/benchmark_messenger_fullcall_lmax300.py` times the actual end-to-end call to get the real number before picking a production setting.
  - **Full end-to-end per-sweep cost, `m_group_size ∈ {1,3,5}` (2026-07-04/07, real Planck data, lmax=300):** job `11562981` (`--mem=32G`) timed `m_group_size=1` (89.62s/sweep) and `m_group_size=3` (188.08s/sweep) before being OOM-killed calibrating `m_group_size=5` — `_calibrate_block_AtA` transiently allocates a `(block_size, npix)` dense array per block (~18GB for the largest near-`m=0` block at `m_group_size=5`, on top of the already-cached `m_group_size=1,3` blocks accumulating in `model._messenger_AtA_blocks_cache` across the loop). Fixed by dropping each group size's cache after timing it (script now does this) and raising the job to `--mem=96G`; rerun as job `11572610` completed cleanly: **`m_group_size=5` → 354.68s/sweep** (mean of 5 timed sweeps, `n_messenger_iter=100`). Cost scales slightly worse than linearly in `m_group_size` (1x/2.1x/4.0x for group sizes 1/3/5), as expected from the `O(size^3)` Cholesky term.
  - **`m_group_size` decision: 5.** The isolated lmax=16 validation sweep (Step 5 above) showed `m_group_size=5` matches the exact dense `A^T A` correction's accuracy (4.93/1.60 SE max/mean) while `m_group_size=1` and `3` are markedly worse (34.13/6.88 and 26.97/6.26 SE) — this is a large, qualitative accuracy gap, not a marginal one. At 354.68s/sweep (~5.9 min), a 72h job (COSMA `dine2`, matching `submit_cg_L300.slurm`'s existing wall-time) fits ~730 sweeps, comfortably enough for burn-in plus several hundred production samples. The "vs CG baseline" cost framing in the benchmark scripts' own output (`MORE EXPENSIVE per unit`) is the wrong comparison, as their own caveat notes: CG's PCG never converged at *any* iteration budget on this masked-sky problem (Phase 0b), so the real comparison is total wall-clock per usable Gibbs sweep — messenger at `m_group_size=5` delivers one in ~6 minutes; CG delivered none.
  - **Production validation chain launched 2026-07-07:** `scripts/submit_messenger_L300.slurm`, 4 chains (array 1-4), `--alm_sampler messenger --messenger_use_block_correction --messenger_m_group_size 5 --n_messenger_iter 100 --use_matrixfree_sht`, lmax=300/NSIDE=256, real Planck data, `n_burnin=20`, `n_samples=520` (checkpointed every 100). Output: `results/lmax300_nside256_gibbs_real_messenger/`. Started fresh (not warm-started from the CG/HMC checkpoint directory, whose `alm_state` provenance is now ambiguous after multiple superseded CG attempts wrote into it — see the `buggy_cg_backup`/`suspect_flatresidual_cg_backup` subdirectories) — a short burn-in should be inexpensive given each alm|C_l draw is a fresh near-exact conditional sample regardless of starting point. **Not yet done:** confirm the chains complete/checkpoint correctly, then verify ESS ≈ N at all multipoles, C_l agreement with the Phase 0 HMC posterior at l ≤ 100, and no flat-residual-equivalent pathology.
