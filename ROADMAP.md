...
- [ ] **Step 5 — resolve the Step 3 stability/bias trade-off** before anything else in this phase: either an exact-quadrature full-sky SHT, or a validated preconditioner that captures the off-diagonal `AᵀA` coupling well enough to avoid both divergence and large bias. Only after that: benchmark/tune at production scale (lmax=300) — cost per messenger sub-iteration vs the 6.7s/PCG-iteration baseline, `n_messenger_iter`, any `τ²`-annealing schedule.
