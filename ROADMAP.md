...
- [ ] Wrap `ducc0.sht` synthesis (alm → map) and adjoint synthesis (map → alm) in a `tf.custom_gradient` op. This is mathematically clean: the derivative of the linear map Y is Y itself, so forward = `synthesis`, backward = `adjoint_synthesis` — the same pairing the codebase already exploits via `_cg_jt_v_fn`, with no stored matrix. ducc0 is multi-threaded C++ and does lmax=1000/NSIDE=1024 transforms in seconds on a CPU node; memory drops from O(lmax² × Npix) to O(Npix)
- [ ] Validate the wrapped op against the dense path: matvec agreement to fp64 tolerance at lmax=50 and lmax=300, plus the existing CG operator symmetry/linearity regression (`tests/test_cg_matvec.py`) run with the new backend
- [ ] Re-run `scripts/benchmark_lensing.py` with the matrix-free backend; **gate: forward+backward at lmax=300 must come in under ~1s (vs 9.38s dense)** before Phase 2 production chains are submitted
- [ ] Keep the dense path behind a flag for regression testing at small lmax
...
- [ ] Replace the inline `.numpy()`/healpy bilinear-interpolation calls in `lens_map_phi_diff_tf` with a TF-native gather-based bilinear interpolation (neighbours + weights precomputable per iteration), so the phi-block HMC step can be `@tf.function`-traced instead of running eagerly
