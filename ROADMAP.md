...
- [ ] Re-run `tests/test_cg_matvec.py`'s symmetry/linearity/PD checks with `use_matrixfree_sht=True` specifically (current coverage: `test_sht_ducc_model_integration.py` checks `psi_tf`/gradient agreement, and a fresh full-scale `debug_cg.py` run — see Phase 0b entry above — confirms symmetry/PD/linearity/CG convergence at lmax=300, but that run used the *dense* path, not `use_matrixfree_sht=True`)
...
- [ ] Make `lens_map_phi_diff_tf` graph-traceable. Scoped down 2026-07-02: the original plan (reimplement HEALPix bilinear interpolation + its weight derivatives analytically in native TF) is a much larger, higher-risk undertaking than it looked — see the phi-block gradient bug flagged in Phase 2 below, discovered while scoping this. The graph-tracing goal itself is separable and lower-risk: wrap the *existing* FD-based numpy/healpy math in `tf.py_function` (same fix already applied to `sht_ducc.py`'s `masked_synthesis_tf`, proven there), which requires no changes to the interpolation math, just threading `neighbors`/`weights`/`theta_lensed`/`phi_lensed` through `tf.py_function` calls instead of closure-captured numpy arrays. Deliberately not done in the same pass as the SHT wiring above — fix the accuracy bug first, or the graph-traceable version just traces the same wrong gradient faster.
...
- **⚠ Pre-existing bug, found 2026-07-02 (unrelated to Phase 1.5 work, discovered while checking test suite health):** `tests/test_lensing.py::test_phi_grad_deflection_adjoint_vs_fd` and `::test_psi_lensed_phi_grad_vs_fd` **fail on current `main`**, up to 81% of checked `dL/dphi_alm` components mismatched vs finite differences (some by >100% relative, occasional sign flips). Bounded root-cause check: individual-pixel `dT_lensed/dtheta_lensed` values agree beautifully between FD step sizes 1e-7 and 1e-5 (agree to ~1e-7 on values ~1e3) for the vast majority of pixels, but a small subset near HEALPix bilinear-interpolation cell boundaries show wild FD-step-size-dependent values (a real, expected discontinuity in `hp.get_interp_weights`'s neighbor-set — already flagged in `lens_map_phi_diff_tf`'s own docstring). The spin-1 SHT adjoint (`_deflection_adjoint`) then smears these few bad-pixel artifacts across most `phi_alm` modes, which is why the test's mismatch count is large despite the per-pixel bug being localized. **Not fixed here** — this needs either (a) excluding/down-weighting boundary-crossing pixels in the FD-based backward pass, or (b) replacing the FD bilinear-weight-derivative with an analytic formula (immune to the discontinuity except exactly at the boundary), which is a properly-scoped follow-up, not a quick patch. Until resolved, **Block 3 HMC gradients should not be trusted for production inference** — this is independent of, and does not block, the Phase 1.5 matrix-free-SHT work above.
