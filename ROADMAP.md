...
*Last substantive revision: 2026-07-03 (Phase 0c messenger-field sampler wired into the real model and into run_gibbs_chain, but masked-sky validation is blocked on a newly-found stability-vs-bias trade-off in the harmonic-space diagonal approximation — see Phase 0c Step 3/5).*
...
- [~] **Step 5 — resolve the Step 3 stability/bias trade-off ▶ (root cause confirmed 2026-07-03, scalable fix still open).** Before building anything scalable, validated the hypothesis cheaply: does capturing the *exact* off-diagonal `AᵀA` (not just a better diagonal) actually fix both divergence and bias, or is something else going on? Added `sample_s_given_t_dense`/`run_messenger_gibbs(..., AtA=...)` (`messenger.py`) — an exact conjugate-Gaussian `s|t` update using the full `AᵀA` matrix via Cholesky, only tractable while `n_alm` is small (O(n_alm³) per draw). Two checks, both confirming the off-diagonal coupling is the actual root cause:
  ...
  - **Consequence:** the exact dense update is not production-viable on its own (`n_alm ≈ 45000` at lmax=300 → `O(n_alm³)` Cholesky per Gibbs step is infeasible), but it de-risks investing in a scalable approximation of the same off-diagonal structure. Two candidate directions, neither built yet: (a) a sparse/low-rank (Woodbury) correction exploiting that off-diagonal `AᵀA` terms are only ~0.75% of the mean diagonal and empirically structured (concentrated in nearby `(l,m)`, not dense-random — worth checking directly before picking a rank), or (b) switch the messenger's internal operator to an exact-quadrature grid (Gauss-Legendre via `ducc0.sht.synthesis_2d(geometry="GL")`, confirmed available) so `AᵀA` is exactly diagonal — at the cost of resampling the real Planck data/mask onto a non-HEALPix grid, a methodological change to the actual likelihood being sampled, not just an implementation detail.
  - Not yet done: characterising the off-diagonal `AᵀA` structure (banded? low-rank? both?) to pick between (a) and (b), and — if (a) is chosen — building and validating the resulting scalable preconditioner at lmax=300. Only after that: benchmark/tune at production scale — cost per messenger sub-iteration vs the 6.7s/PCG-iteration baseline, `n_messenger_iter`, any `τ²`-annealing schedule.
- [ ] **Step 6 — production validation chain** at lmax=300, real Planck data: verify ESS ≈ N, C_l agreement with the Phase 0 HMC posterior, and no flat-residual-equivalent pathology.
...
**Output:** an alm|C_l sampler that actually converges on the real masked-sky problem, unblocking Phase 0b and the rest of the critical path. **Not yet achieved** — Step 3's stability-vs-bias trade-off is the new critical-path blocker, replacing the diagonal-preconditioned-CG failure it was meant to fix.
