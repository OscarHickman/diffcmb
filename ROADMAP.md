...
- [x] Validated the fix in isolation: `scripts/verify_cg_matvec.py` + `scripts/debug_cg.py` (job 11552527, 2026-07-01) — A is now symmetric (1.8e-12 rel. error), positive-definite, linear (3.6e-13 rel. error), and PCG residual decreases monotonically (2.5e7 → 2.1e5 over 10 iters)
- [ ] Re-run production CG chains (job 11513133 superseded) with the fix and verify: ESS ≈ N at all multipoles including l=200–300 — resubmitted as job 11552530 (array 1-4) on dine2, 2026-07-01. Checkpoints truncated back to the 1000 clean Phase 0 HMC samples first; the ~600-700 CG samples job 11513133 collected before timing out were drawn with the broken operator and discarded (backed up to `results/lmax300_nside256_gibbs_real_double/buggy_cg_backup/`)
