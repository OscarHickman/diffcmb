...
### T0.1c status (updated 2026-09-24 afternoon)

**c1: a_ℓm null DONE. The a_ℓm `[30,60)` offset is the statistic, not the sampler.**
The first run, job 12041984, had a bug in the effective-noise calibration (z ~ 10¹⁸
in the low bins). It was fixed and re-run as **job 12042747**
(`logs/null_alm_rank_12042747.out`; `<ensemble>/null_alm_power_rank_flat_prior.npz`).

| a_ℓm rank mean_u | bin | observed | effective null | z | nominal-null z |
|---|---|---|---|---|---|
| Block-4-OFF | `[2,10)` | 0.463 | 0.471 | −0.14 | −0.47 |
| | `[10,30)` | 0.368 | 0.399 | −0.53 | −1.53 |
| | `[30,60)` | 0.273 | 0.349 | **−1.35** | −2.58 |
| | `[60,64)` | 0.601 | 0.461 | +2.36 | +1.95 |
| Block-4-ON | `[2,10)` | 0.483 | 0.470 | +0.22 | −0.13 |
| | `[10,30)` | 0.359 | 0.394 | −0.62 | −1.70 |
| | `[30,60)` | 0.280 | 0.341 | **−1.09** | −2.45 |
| | `[60,64)` | 0.632 | 0.458 | +2.92 | +2.45 |

Every bin is |z| < 3 against the effective null, so **the three-block core is not
rejected by the a_ℓm row on the exact operator.** Watch `[60,64)`, marginal in
both ensembles (+2.4 / +2.9) and offset the other way. Figure 1's a_ℓm row is now
drawn and tested against this null (`fig1_validation.py --alm_null effective`,
the default); see the job 12047933 figure numbers below.

**c2/c3: the first long runs all FAILED; resubmitted.** Jobs 12041916 and
12041917: all 48 tasks died at their first checkpoint write, ~3 h in
(`FileNotFoundError: results/analysis/..._long/chain_rNNN_ckpt.npz`). Commit
`115e6da` (02:05) turned `results/analysis` into a symlink to `/cosma5` while the
jobs were running on `dine2`, which does not mount `/cosma5`. `sacct` showed
COMPLETED because the wrapper swallowed the exit code. No chain data survived.
(Also: 12041916 was **ν = 30** and 12041917 **ν = 6**, the reverse of what this
table said before.) `submit_ensemble_exact_lmax64.slurm` now runs on the **/cosma5-mounting partitions** (cosma5, cosma8-shm/shm2/shm3, cosma8-ska)
(36 h wall; the chains are CPU-only, since TF on dine2 logged
`CUDA_ERROR_NO_DEVICE`) and exits with Python's status.

| job | what | output | expected |
|---|---|---|---|
| 12047785 | Block-4-ON, ν = 6, **1000 burn-in + 3600 samples** (same skies r000–r023) | `ens_exact_l64_A3000_n30_long/` | ~13–15 h/task once running |
| 12047786 | Block-4-ON, **ν = 30**, same length, same skies | `ens_exact_l64_A3000_n30_nu30_long/` | same |

⚠ Every other `dine2` wrapper that writes under `results/analysis/` has the same
problem (about 30 in `scripts/`). Move each one to a `/cosma5`-mounting partition
before reusing it.

**How to read them:**
- **Null (12041984 → 12042747, done: pass).** Compare each bin's observed mean_u with the *effective*
...
- **Long ν = 6 (12047785; 12041916 was the failed ν = 30 run).** Re-run `diagnose_calibration_stationarity.py`,
...
- **ν = 30 (12047786).** If the φ `[30,60)` rank offset (0.70 at ν = 6) shrinks
...
mean_u < 0.5). **Confirmed by the null (job 12042747, above).**
...
- **Low-L φ `[2,10)` is not stationary.** Quarter mean_u 0.771 / 0.717 / 0.614 / 0.661;
  ...
  burn-in from the MAP start, so run longer (job 12047785). Block-4-OFF φ `[2,10)`
...
**Next:** read jobs 12047785 / 12047786 as described under "How to read
them" above (c1 is done). The first-harvest numbers in this table are superseded by
