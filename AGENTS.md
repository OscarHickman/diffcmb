...
**History to 2026-09-25:** everything before that date used the legacy forward model. Three exact-operator ensembles (amplified lensing A_φ = 3000, σ = 30 μK/pixel, lmax = 64) are complete and were first harvested 2026-09-23: physics clean, **calibration fails** (joint-likelihood SBC rejected on Block-4-ON; a_ℓm `[30,60)` rank offset in both ensembles; a_ℓm ESS ~35/1200). `ROADMAP.md` → T0.1 (diagnose it) is blocking. Steps (a, b) were harvested 2026-09-24 and found two separate failures: a stationary a_ℓm `[30,60)` offset in both ensembles, suspected to be the statistic; and in Block-4-ON, low-L φ burn-in plus a φ `[30,60)` offset. Step (c1), the a_ℓm null, passed (job 12042747): the a_ℓm offset is the statistic. Steps (c2, c3) were harvested 2026-09-25 (job 12062700) from the long chains: 1000 + 3600 sweeps, ν = 6 (job 12047785) and ν = 30 (job 12047786).
- At ν = 6 the low-L φ burn-in is gone, but the φ `[30,60)` offset remains.
- At ν = 30 every calibrated test passes.
- The ν-dependence points to mixing of the centred (φ, C_L^φφ) hierarchy (a funnel).

**Decision D5 (2026-09-25): the paper's Block-4-ON ensemble is `ens_exact_l64_A3000_n30_nu30_long` (ν = 30)**, with its blind pair (job 12062768). The Makefile `ENSEMBLE` points there, and all figures were rebuilt from it (job 12062824). Next is the per-figure checklist read (`ROADMAP.md` T0.2). `results/analysis` is a symlink into `/cosma5`, which dine2 does not mount, so **don't run anything that writes there on dine2**. Numbers and how to read them: `results/analysis/dashboard.md`. The `exact-operator-rerun` branch has been merged; work is on `main`.

**▶ RESUME HERE (state 2026-09-26):** T0.2 figure checks are done at N = 24: figure 1 at thin 50 passes, figure 2's `[30,45)` residual matches the exact-sampler expectation (not zero), and figure 4's two cells are chance. **Waiting on 24 fresh skies r024–r047 at ν = 30: Block-4-ON job 12065119, submitted 2026-09-26 ~19:15, about 13–16 h per task.** The blind pair, job 12065120, is already done (24/24). When they finish, follow the harvest checklist in `docs/dashboard.md` → "CURRENT (2026-09-26)" → "Harvest checklist (N = 48)". Then there are three decisions for the user (`ROADMAP.md` T0.2): the figure 2 reference band, the Block-4-OFF panel, and the power table.
...
- **`docs/dashboard.md`** (also reachable as `results/analysis/dashboard.md`, a symlink back into the repo, so it stays version-controlled although `results/analysis/` lives on `/cosma5`) — live status of the production chains: current headline SBC numbers, what is in flight, which output directories are invalid, and how to read the spectrum rows. Check it before harvesting or citing any chain.
