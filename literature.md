...
- [ ] **Add the "what the deficit is not" paragraph** (not N0/N1, not mean-field, not non-Gaussian deflection, not foregrounds). Note that one candidate on that list is now *removed rather than excluded*: the missing `Im(a_{ℓ,1})` dof was restored 2026-09-06, so it can no longer be offered as an explanation for the low-ℓ φ deficit — nor was it ever measured to be one.
